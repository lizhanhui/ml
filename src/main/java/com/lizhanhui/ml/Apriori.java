...
public class Apriori {
    ...
    private final static String ITEM_SEPARATOR = ";"; // 项之间的分隔符
        ...
        while (itemkFcMap.size() != 0) {
            ...
            for (String trans : transList) {
                for (String candidate : ccKeySet) {
                    ...
                    String[] candidateItems = candidate.split(ITEM_SEPARATOR);
                    for (String candidateItem : candidateItems) {
                        if (!trans.contains(candidateItem + ITEM_SEPARATOR)) {
                            flag = false;
                            break;
                        }
                    }
    ...
    private Map<String, Integer> getCandidateCollection(Map<String, Integer> itemkFcMap) {
        ...
        for (String itemk1 : itemkSet1) {
            for (String itemk2 : itemkSet2) {
                //进行连接
                String[] tmp1 = itemk1.split(ITEM_SEPARATOR);
                String[] tmp2 = itemk2.split(ITEM_SEPARATOR);
                String c = "";
                if (tmp1.length == 1) {
                    if (tmp1[0].compareTo(tmp2[0]) < 0) {
                        c = tmp1[0] + ITEM_SEPARATOR + tmp2[0] + ITEM_SEPARATOR;
                    }
                } else {
                    boolean flag = true;
                    ...
                    if (flag && (tmp1[tmp1.length - 1].compareTo(tmp2[tmp2.length - 1]) < 0)) {
                        c = itemk1 + tmp2[tmp2.length - 1] + ITEM_SEPARATOR;
                    }
                }
                ...
                if (!c.equals("")) {
                    String[] tmpC = c.split(ITEM_SEPARATOR);
                    for (int i = 0; i < tmpC.length; i++) {
                        String subC = "";
                        for (int j = 0; j < tmpC.length; j++) {
                            if (i != j) {
                                subC = subC + tmpC[j] + ITEM_SEPARATOR;
                            }
                        }
    ...
    private Map<String, Integer> getItem1FC() {
        ...
        for (String trans : transList) {
            String[] items = trans.split(ITEM_SEPARATOR);
            for (String item : items) {
                Integer count = sItem1FcMap.get(item + ITEM_SEPARATOR);
                if (count == null) {
                    sItem1FcMap.put(item + ITEM_SEPARATOR, 1);
                } else {
                    sItem1FcMap.put(item + ITEM_SEPARATOR, count + 1);
                }
            }
        }
    ...
    public Map<String, Double> getRelationRules(Map<String, Integer> frequentCollectionMap) {
        ...
        for (String key : keySet) {
            ...
            String[] keyItems = key.split(ITEM_SEPARATOR);
                ...
                for (List<String> itemList : result) {
                    ...
                    if (itemList.size() < source.size()) {//只处理真子集
                        ...
                        for (String item : itemList) {
                            ...
                            reasonStr = reasonStr + item + ITEM_SEPARATOR;

                        }

                        for (String item : otherList) {

                            resultStr = resultStr + item + ITEM_SEPARATOR;
